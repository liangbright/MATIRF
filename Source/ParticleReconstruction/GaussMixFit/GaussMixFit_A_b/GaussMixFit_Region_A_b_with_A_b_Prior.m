function [A b beta]=GaussMixFit_Region_A_b_with_A_b_Prior(x_Init, y_Init, A_Init, R_Init, b_Init, Range_A, Range_b, ...                                                    
...
for Iteration=1:MaxIteration
    ...
    Coef=zeros(PixelNum+2*ParticleNum, 2*ParticleNum);
    ...
    WA_Prior=1./(A_SigmaPrior+1);
    
    %--------------------------------------------------------------------------------------------
    for k=1:2*ParticleNum
        Coef(1:PixelNum,k)=Coef(1:PixelNum,k).*WI;          
    end    
    Coef(PixelNum+1:PixelNum+ParticleNum, 1:ParticleNum)=diag(WA_Prior);
    Coef(PixelNum+ParticleNum+1:PixelNum+2*ParticleNum, ParticleNum+1:2*ParticleNum)=diag(Wb_Prior);   
    
    Data=[I_Data.*WI; A_Prior.*WA_Prior; Ib_Prior.*Wb_Prior];
