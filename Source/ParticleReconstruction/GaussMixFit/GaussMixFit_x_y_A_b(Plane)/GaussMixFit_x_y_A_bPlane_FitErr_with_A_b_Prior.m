function [F J Ib]=GaussMix_FitErr_with_A_b_Prior(Param, ParticleNum, PSFSigma, I_Data, X, Y, ...
...
y=Param(sIdx:eIdx);
%---------------------------------------------------------------------------------------------
F=zeros(2*PixelNum+ParticleNum, 1);
J=zeros(2*PixelNum+ParticleNum, 6*ParticleNum);
%%
...
dFdA=zeros(2*PixelNum+ParticleNum,ParticleNum);    
dFdx=zeros(2*PixelNum+ParticleNum,ParticleNum);    
dFdy=zeros(2*PixelNum+ParticleNum,ParticleNum);    
dFdP0=zeros(2*PixelNum+ParticleNum,ParticleNum);
dFdP1=zeros(2*PixelNum+ParticleNum,ParticleNum);
dFdP2=zeros(2*PixelNum+ParticleNum,ParticleNum);        
...
for k=1:ParticleNum    
    ...
    % Cost=sum((A-A_Prior).^2)
    sIdx=2*PixelNum+1; eIdx=2*PixelNum+ParticleNum;
    dFdA(sIdx:eIdx,k)=WA;
end
...
F(2*PixelNum+1:2*PixelNum+ParticleNum)=(A-A_Prior).*WA;  
%--------------------------------------------------------------------------------
    
%--------------------------------------------------------------------------------
sRow=1; eRow=2*PixelNum+ParticleNum;    
%--------------------------------------------------------------------------------
