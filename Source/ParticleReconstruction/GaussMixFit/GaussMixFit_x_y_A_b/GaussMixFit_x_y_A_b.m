function [x y A b A_std Region] = GaussMixFit_x_y_A_b(x_Init, y_Init, A_Init, R_Init, b_Init, Radius_xy, Range_A, Range_b, ...                                                    
                                                      OtherParam, ...
...
for n=1:RegionNum
    ...
    if isempty(Ib_Prior) && isempty(A_Prior)
        ...
        [x_n y_n A_n b_n Param_n] = GaussMixFit_Region_x_y_A_b(x_n, y_n, A_n, R_n, b_n, Radius_xy_n, Range_A_n, Range_b_n, ...
                                                               ImageInfo, RegionInfo, I_Data);    
                                                           
    elseif ~isempty(Ib_Prior) && isempty(A_Prior)
        Ib_Prior_n=Ib_Prior(PixelIdxList_n);    
        Ib_SigmaPrior_n=Ib_SigmaPrior(PixelIdxList_n);
    ...
    [A_std_n MSE_n]=GaussMixFit_x_y_A_b_STATISTICS(Param_n, ParticleNum_n, R_n, I_Data, RegionInfo.XPixelList, RegionInfo.YPixelList);    
    %% Output x_n y_n a_n            
    x(ParticleIdx_n)=x_n;
    y(ParticleIdx_n)=y_n;
    A(ParticleIdx_n)=A_n;
